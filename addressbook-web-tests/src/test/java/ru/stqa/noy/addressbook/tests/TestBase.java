...
public class TestBase {
  ...
  public void logTestStart(Method m, Object[] p) {
    logger.info("Start test " + m.getName() + " with parameters " + Arrays.asList(p));    //info - уровень сообщения (debug, error и т.д.)
  }
  ...
  public void verifyContactListInUI() {
    if (Boolean.getBoolean("verifyUI")) {
      ...
      assertThat(uiContacts, equalTo(dbContacts.stream().map((c) -> new AddNewData()
              .withId(c.getId()).withLastname(c.getLastname()).withFirstname(c.getFirstname()))
              .collect(Collectors.toSet())));
    }
  }
}